...
import shutil, tempfile, zipfile, subprocess
...
if not (src/'include'/'registries.h').exists() and (src/'notchian').exists() and (src/'build_registries.js').exists():
    subprocess.run(['node','build_registries.js'],cwd=src,check=True)
for p in (src/'include').glob('*.h'): shutil.copy2(p,OUT/p.name)
for p in (src/'src').glob('*.c'): shutil.copy2(p,OUT/p.name)
g=OUT/'globals.h'; t=g.read_text().replace('#define MAX_PLAYERS 16','#define MAX_PLAYERS 4').replace('#define MAX_BLOCK_CHANGES 20000','#define MAX_BLOCK_CHANGES 4000'); g.write_text(t)
...
m=OUT/'main.c'; t=m.read_text(); t=t.replace('int main () {','int bareiron_server_main () {',1); t=t.replace('main();','bareiron_server_main();',1); t=t.replace('int clients[MAX_PLAYERS], client_index = 0;','int client_index = 0;',1); t=t.replace('clients[i] = -1;','bareiron_clients[i] = -1;',1); start=t.find('int client_index = 0;'); a=t[:start]; b=t[start:]; b=b.replace('clients[','bareiron_clients['); t=a+b; t=t.replace('while (true) {','while (bareiron_should_run) {',1); t=t.replace('\n  close(server_fd);','\n  for (int i=0;i<MAX_PLAYERS;i++){if(bareiron_clients[i]!=-1)close(bareiron_clients[i]);bareiron_clients[i]=-1;}\n  close(server_fd);',1); t=t.replace('#include "globals.h"','#include "globals.h"\n#include "bareiron_arduino.h"\nextern volatile uint8_t bareiron_should_run;\nextern int bareiron_clients[MAX_PLAYERS];',1); m.write_text(t)
...
t=rep(t,'uint8_t getHeightAtFromAnchors (int rx, int rz, ChunkAnchor *anchor_ptr)','''uint8_t getHeightAtFromAnchors (int rx,int rz,ChunkAnchor *anchor_ptr){if(rx==0&&rz==0){int h=rw_h(anchor_ptr[0].x,anchor_ptr[0].z,anchor_ptr[0].biome);if(h>67)return h-1;}return interpolate((uint8_t)rw_h(anchor_ptr[0].x,anchor_ptr[0].z,anchor_ptr[0].biome),(uint8_t)rw_h(anchor_ptr[1].x,anchor_ptr[1].z,anchor_ptr[1].biome),(uint8_t)rw_h(anchor_ptr[16/CHUNK_SIZE+1].x,anchor_ptr[16/CHUNK_SIZE+1].z,anchor_ptr[16/CHUNK_SIZE+1].biome),(uint8_t)rw_h(anchor_ptr[16/CHUNK_SIZE+2].x,anchor_ptr[16/CHUNK_SIZE+2].z,anchor_ptr[16/CHUNK_SIZE+2].biome),rx,rz);}''')
w.write_text(t)
